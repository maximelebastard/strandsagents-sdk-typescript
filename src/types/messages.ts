...
export class ToolResultBlock implements ToolResultBlockData {
  ...
  constructor(data: ToolResultBlockData) {
    this.toolUseId = data.toolUseId
    this.status = data.status
    this.content = data.content.map((content) => {
      if ('text' in content) {
        return new TextBlock(content)
      } else if ('json' in content) {
        return new JsonBlock(content)
      } else {
        throw Error('Unsupported content type for ToolResultBlock')
      }
    })
  }
}
